...
const validateSourceMap = (
  ...
  const leavesWithString: SourceMap[] = leaves.filter(
    (leaf) => leaf.string !== undefined
  );
...
const absolutifySourceMap = (
  sourceMap: SourceMap,
  offset: number = 0
): SourceMap => {
  const newOffset = sourceMap.start + offset;
  const children =
    sourceMap.children?.map((child) => absolutifySourceMap(child, newOffset)) ||
    [];
  return {
    ...sourceMap,
    start: newOffset,
...
describe("sourcemap", () => {
  function TestPromptTrivial(
    props: PromptProps<{ message: string }>
  ): PromptElement {
    return (
      <>
  ...
  function TestPromptEmojiAndJapanese(): PromptElement {
    return (
      <>
        ...
        <UserMessage>
          <scope name={"lemon"}>🍋</scope>
          <scope name={"japanese"}>これはレモン</scope>
        </UserMessage>
      </>
    );
  }

  function TestSourceMapMoreComplex(
    props: PromptProps<{ message: string }>
  ): PromptElement {
      ...
      <>
        ...
        <UserMessage p={1000}>
          <scope name={"the first line"}>
            ...
            <br />
            <scope name={"buffer line"}>{lines[0]}</scope>
            <br />
          </scope>
        ...
        <UserMessage>
          ...
          <scope name={"checks"} prel={-10}>
            const promptString =
            Priompt.promptToString_VULNERABLE_TO_PROMPT_INJECTION(rendered.prompt,
            getTokenizerByName("cl100k_base")) const sourceMap =
            rendered.sourceMap
          </scope>
          <scope name={"expects"}>
            expect(sourceMap).toBeDefined() expect(sourceMap?.start).toBe(0)
            <scope name={"lets put the mesage lines here again"}>
              {props.message.split("\n").map((line, i) => (
                <scope name={`line ${i}`}>{line}</scope>
              ))}
            </scope>
            expect(sourceMap?.end).toBe(promp
            <scope name="suspicious inline scope">tString.length)</scope>
            expect(
  ...
  it("should generate simple sourcemap correctly", async () => {
    const rendered = await render(
      TestPromptTrivial({ message: "System message for sourcemap test." }),
      {
        tokenLimit: 1000,
    ...
    const promptString = Priompt.promptToString_VULNERABLE_TO_PROMPT_INJECTION(
      rendered.prompt,
